...
class Nguon_avdbapi_crawler
{
        ...
        try {
            ...
            if ($my_posts) { 
                ...
                if (!$this->OVERIDE_UPDATE) {
                    $result = array(
                        'code' => 999,
                        'message' => $data["slug"] . ' : No need to update',
                        'data' => $movie_data
                    );
    ...
    private function insert_movie($data)
    {
        ...
        $video_embed = array_shift(array_values($data["episodes"]["server_data"]));
        $multiLinks = array(
            0 => array(
                "ml_label" => "Multi-Links",
                "ml_url" => ""
            )
        );;
        $casts = array();
        ...
        if (count($data["episodes"]["server_data"]) > 1) {
            $first = true;
            foreach ($data["episodes"]["server_data"] as $key => $val) {
                if ($first) {
                    $first = false;
                    continue; // Skip appending for the first element
                }
                $multiLinks[0]["ml_url"] .= $val["slug"] . "\n" . '<iframe src="' . $val['link_embed'] . '" frameborder="0" scrolling="no" width="960" height="720" allowfullscreen></iframe>'. "\n";
            }
        }
        ...
        foreach ($data['category'] as $category) {
            ...
            $term = get_term_by("name", $category, 'vidmov_video_category');
            if ($term && !is_wp_error($term)) {
                $term_id = $term->term_id;
                wp_set_post_terms($post_id, $term_id, 'vidmov_video_category', false);
            }
        }
        ...
        foreach ($data['actor'] as $actor) {
            if (!term_exists($actor) && $actor != '') {
                $args_casts = array(
                    'name' => $actor,
                    'post_type' => 'vidmov_cast',
                    'post_status' => 'publish',
                    'posts_per_page' => 1
                );
                $my_casts = get_posts($args_casts);
                if ($my_casts) {
                    $casts[] = $my_casts[0]->ID;
                } else {
                    $cast_data = array(
                    ...
                    $cast_id = wp_insert_post($cast_data);
                    $casts[] = $cast_id;
                }
            }
        }
        ...
        foreach ($data['director'] as $director) {
            if (!term_exists($director) && $director != '') {
                $args_crews = array(
                    'name' => $director,
                    'post_type' => 'vidmov_crew',
                    'post_status' => 'publish',
                    'posts_per_page' => 1
                );
                $my_crews = get_posts($args_crews);
                if ($my_crews) {
                    $crews[] = $my_crews[0]->ID;
                } else {
                    $crew_data = array(
                    ...
                    $crew_id = wp_insert_post($crew_data);
                    $crews[] = $crew_id;
                }
            }
        }
        ...
        //add year
        wp_set_post_terms($post_id, $data['year'], 'post_tag', true);

        //add country
        foreach ($data['country'] as $country) {
            wp_set_post_terms($post_id, $country, 'post_tag', true);
        }
        wp_set_post_terms($post_id, $data['tag'], 'post_tag', true);

        wp_set_post_terms($post_id, $data['movie_code'], 'post_tag', true);
        ...
        if (isset($data['poster_url']) && $this->CRAWL_IMAGE != 0) {
            ...
            $post_meta_movies = array(
                ...
                'beeteam368_video_tag_label' => $data['quality'],
                'beeteam368_multi_links' => $multiLinks,
                'vidmov_cast_include' => $casts,
                'vidmov_crew_include' => $crews,
                'beeteam368_video_url' => '<iframe src="' . $video_embed['link_embed'] . '" frameborder="0" scrolling="no" width="960" height="720" allowfullscreen></iframe>',
                'beeteam368_video_duration' => $duration,
